    ...
    if request.method == 'POST':
        ...
        if user is not None:
            if not user.is_active:
                messages.warning(request,
                                 "Votre compte n'est pas encore activé, Veuiller l'activer en cliquant sur le lien vous a été envoyé via mail. <strong><a href='/main/confirm_mail/resend'>Renvoyer l'email</a></strong> ")
                return redirect('main_app:log_in')
...
def confirm_mail_resend(request): #Vue utilisée quand on clique sur Renvoyer un autre email et qu'on tape l'email dans le formulaire, on le recupere par get
    ...
    return render(request, 'authentification/confirm_mail_form.html')
